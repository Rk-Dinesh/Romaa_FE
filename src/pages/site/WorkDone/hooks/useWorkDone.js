...
const fetchWorkDoneSummary = async (tenderId) => {
  const { data } = await api.get(`/workdone/api/summary/${tenderId}`);
  return data?.data || [];
};

export const useWorkDoneSummary = (tenderId) => {
  return useQuery({
    queryKey: ["workdone-summary", tenderId],
...
const fetchWorkDoneDetail = async (tenderId, workDoneId) => {
  const { data } = await api.get(`/workdone/api/details/${tenderId}/${workDoneId}`);
  return data?.data || null;
};

export const useWorkDoneDetail = (tenderId, workDoneId) => {
  return useQuery({
    queryKey: ["workdone-detail", tenderId, workDoneId],
...
const fetchWorkDoneByDate = async (tenderId, reportDate) => {
  const { data } = await api.get(`/workdone/api/report-date/${tenderId}/${reportDate}`);
...
export const useWorkDoneByDate = (tenderId, reportDate) => {
  return useQuery({
    queryKey: ["workdone-by-date", tenderId, reportDate],

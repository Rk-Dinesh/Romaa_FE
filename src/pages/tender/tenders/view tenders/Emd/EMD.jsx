    ...
    <div className="pb-10 font-layout-font animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Total EMD Required</p>
            <p className="text-xl font-bold text-gray-800 dark:text-white">{fmt(emdData?.emd?.emd_amount)}</p>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Approved Bidder</p>
            <p className="text-sm font-bold text-gray-800 dark:text-white truncate max-w-[150px]">
              {emdData?.emd?.approved_emd_details?.emd_proposed_company || "None Approved"}
            </p>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-gray-800 dark:text-white">
              {emdData?.emd?.emd_validity ? new Date(emdData.emd.emd_validity).toLocaleDateString("en-GB") : "Not Set"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        ...
        <button
          ...
          className="w-full md:w-auto flex items-center justify-center gap-2 bg-darkest-blue text-white px-6 py-2.5 rounded-lg font-bold hover:bg-blue-900 transition-all shadow-md active:scale-95"
        >
      ...
      {/* Proposals Card Grid */}
      {loading ? (
        <div className="py-20 flex justify-center italic text-gray-400">Loading proposals...</div>
      ) : items.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {items.map((item) => (
            ...
            >
              {/* Ranking Badge */}
              <div className={`absolute top-0 left-0 px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-br-xl ${
                item.level === "L1" ? "bg-amber-400 text-amber-900" : "bg-gray-100 text-gray-500 dark:bg-gray-800"
              }`}>
              ...
              <div className="p-6 pt-10">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-400 font-mono mt-0.5">{item.proposal_id}</p>
                  </div>
                  <div className={`px-2.5 py-1 rounded-full text-[10px] font-bold border ${getStatusStyle(item.status)}`}>
                    {item.status}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div>
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide block mb-1">Bid Value</label>
                    <p className="font-bold text-gray-700 dark:text-gray-200">{fmt(item.proposed_amount)}</p>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide block mb-1">EMD Details</label>
                    <p className="font-semibold text-gray-700 dark:text-gray-200">{fmt(item.emd_amount)}</p>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide block mb-1">Bank</label>
                    <p className="text-xs text-gray-600 dark:text-gray-400 truncate">{item.payment_bank || "—"}</p>
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-2 pt-4 border-t border-gray-50 dark:border-gray-800">
                  {item.status === "PENDING" ? (
                    ...
                    <div className="flex-1 flex items-center justify-center gap-2 p-2 bg-gray-50 dark:bg-gray-800 text-gray-400 rounded-lg text-xs font-bold italic">
                      <IoAlertCircleOutline size={18} /> Rejected {item.rejection_reason ? `(${item.rejection_reason})` : ""}
                    </div>
                  )}
      ...
      {/* Modals */}
      {showAddModal && <AddEMD onclose={() => setShowAddModal(false)} onSuccess={() => { fetchProposals(); fetchEMDSummary(); }} />}
      {editItem && <EditEMD onclose={() => setEditItem(null)} item={editItem} onUpdated={() => { fetchProposals(); fetchEMDSummary(); }} />}
      {deleteId && (
        <DeleteModal

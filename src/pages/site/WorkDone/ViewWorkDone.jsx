...
import { ArrowLeft, CalendarDays, Hash, FileText, ClipboardList } from "lucide-react";
import { useWorkDoneByDate } from "./hooks/useWorkDone";
...
const ViewWorkDone = () => {
  ...
  if (!records.length) return (
    <div className="p-10 text-center text-red-500 text-sm">No records found for this date.</div>
  );
    ...
    <div className="font-roboto-flex min-h-screen dark:bg-[#0b0f19] p-6 pb-20">
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <InfoCard icon={<CalendarDays size={20} />} color="blue"   label="Report Date"   value={formatDate(reportDate)} />
          <InfoCard icon={<ClipboardList size={20} />} color="orange" label="Work Orders"  value={`${records.length} order${records.length !== 1 ? "s" : ""}`} />
          <InfoCard icon={<Hash size={20} />}          color="emerald" label="Total Items"  value={`${totalItems} entr${totalItems !== 1 ? "ies" : "y"}`} />
        </div>

        {/* ── One card per work order ── */}
        {records.map((record) => (
          <WorkOrderCard key={record.workDoneId || record.workOrder_id} record={record} />
        ))}
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm">
      ...
      <div className="px-6 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-400 font-mono">{record.workDoneId}</span>
        </div>
        <div className="flex items-center gap-3">
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
            {record.dailyWorkDone?.length === 0 ? (
              <tr>
                <td colSpan="9" className="py-8 text-center text-gray-400 text-sm">No items.</td>
              </tr>
            ) : (
              record.dailyWorkDone?.map((item, i) => (
                <tr key={i} className="hover:bg-blue-50/20 dark:hover:bg-gray-700/20 transition-colors">
          ...
          <tfoot className="bg-gray-50 dark:bg-gray-900/50 border-t-2 border-gray-200 dark:border-gray-700">
            <tr>
              <td colSpan="4" />
              <td className="px-3 py-2 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Total Qty</td>

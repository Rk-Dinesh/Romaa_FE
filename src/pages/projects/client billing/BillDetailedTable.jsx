    ...
    return details.map((row, idx) => (
      <React.Fragment key={row._id || idx}>
        <tr className="hover:bg-gray-50 border-b border-gray-200 text-sm">
          ...
          <td
            className={`p-2 border-r border-gray-300 ${level === 0 ? "font-bold text-gray-800" : "pl-8 text-gray-600"}`}
          >
            {row.description}
          </td>
          <td className="p-2 border-r border-gray-300 text-center">
          ...
          <td className="p-2 border-r border-gray-300 text-right">
            {formatQty(row.breadth)}
          </td>
          <td className="p-2 border-r border-gray-300 text-right">
            {formatQty(row.depth)}
          </td>
          <td
            className={`p-2 border-r border-gray-300 text-right ${level === 0 ? "font-bold" : ""}`}
          >
            {formatQty(row.quantity)}
          </td>
          <td className="p-2 border-r border-gray-300 text-center">
            {level === 0 && (
              <span className="text-xs font-bold">{row.unit || "M3"}</span>
            )}
          </td>
        ...
        {row.details &&
          ...
          renderDetails(row.details, level + 1)}
      </React.Fragment>
    ));
  };
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse min-w-[1200px]">
            <thead className="bg-green-100/50 text-gray-900 font-bold border-b-2 border-gray-400">
              <tr>
                <th className="p-2 border-r border-gray-400 w-16 text-center">
                  Code
                </th>
                <th className="p-2 border-r border-gray-400 text-left min-w-[300px]">
                  Description
                </th>
                <th className="p-2 border-r border-gray-400 w-16 text-center">
                  Nos
                </th>
                <th className="p-2 border-r border-gray-400 w-20 text-right">
                  Length
                </th>
                <th className="p-2 border-r border-gray-400 w-20 text-right">
                  Breadth
                ...
                <th className="p-2 border-r border-gray-400 w-24 text-right bg-green-200/50">
                  Quantity
                </th>
                <th className="p-2 border-r border-gray-400 w-16 text-center">
                  Unit
            ...
            <tbody>
              {Object.entries(groupByDay(detailedData.items)).map(
                ([day, items]) => (
                  <React.Fragment key={day}>
                    <tr className="bg-blue-100/80 border-b border-gray-300">
                      <td
                        colSpan="11"
                        className="p-2 font-bold text-blue-800 uppercase tracking-wide"
                      >
                        {day}
                      </td>
                    </tr>

...
import {
  ArrowLeft, CalendarDays, Users, IndianRupee,
  ClipboardList, HardHat, FileText, UserCheck,
} from "lucide-react";
...
const ViewDailyReportSite = () => {
  ...
  const totals = useMemo(() => ({
    ...
    workers: records.reduce((s, r) => s + (r.attendance_entries?.length || 0), 0),
  }), [records]);
  ...
  if (!records.length) return (
    <div className="p-10 text-center text-sm text-red-500">No reports found for this date.</div>
  );
    ...
    <div className="font-roboto-flex min-h-screen dark:bg-[#0b0f19] p-6 pb-20">
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Daily Labour Report</h1>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {item.project_name || tenderId} · {records.length} contractor{records.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <SummaryCard icon={<CalendarDays size={18} />} color="blue"   label="Report Date"  value={formatDate(reportDate)} />
          <SummaryCard icon={<ClipboardList size={18} />} color="indigo" label="Contractors"  value={records.length} />
          <SummaryCard icon={<Users size={18} />}         color="purple" label="Man Days"     value={totals.manDays.toFixed(1)} />
          <SummaryCard icon={<IndianRupee size={18} />}   color="green"  label="Total Wages"  value={`₹${fmt(totals.amount)}`} />
        </div>
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm">
      ...
      <div className="px-5 py-3 bg-gray-50 dark:bg-gray-900/60 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {record.remark && record.remark !== "No Remark" && (
              <p className="text-xs text-gray-400 italic mt-0.5">{record.remark}</p>
            )}
          </div>
        </div>
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${status.cls}`}>
            {status.label}
          </span>
        </div>
      </div>
      ...
      <div className="divide-y divide-gray-100 dark:divide-gray-700/50">
        ...
        {record.work_entries?.length > 0 && (
          <div className="p-4">
            <SectionTitle icon={<FileText size={13} />} title={`Work Entries (${record.work_entries.length})`} />
              ...
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700/40">
                  {record.work_entries.map((e, i) => (
                    <tr key={i} className="hover:bg-blue-50/20 dark:hover:bg-gray-700/20">
                      <td className="px-3 py-2 font-medium text-gray-800 dark:text-gray-200">{e.description}</td>
                      ...
                      <td className="px-3 py-2 text-center text-gray-500">{e.l || "—"}</td>
                      <td className="px-3 py-2 text-center text-gray-500">{e.b || "—"}</td>
                      <td className="px-3 py-2 text-center text-gray-500">{e.h || "—"}</td>
                      <td className="px-3 py-2 text-right font-bold text-blue-600 dark:text-blue-400">{e.quantity ?? "—"}</td>
                      <td className="px-3 py-2 text-gray-500 uppercase">{e.unit}</td>
                      <td className="px-3 py-2 text-gray-400 italic">{e.remark && e.remark !== "No Remark" ? e.remark : "—"}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot className="bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
                  <tr>
                    <td colSpan="5" className="px-3 py-2 text-right text-gray-500 font-semibold uppercase text-[10px] tracking-wider">
                      Total Qty
                    </td>
        ...
        {record.attendance_entries?.length > 0 && (
          <div className="p-4">
            <SectionTitle icon={<UserCheck size={13} />} title={`Attendance (${record.attendance_entries.length} workers)`} />
            <div className="mt-2 overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
              <table className="w-full text-xs text-left">
                <thead className="bg-gray-50 dark:bg-gray-900/50 text-gray-500 uppercase font-semibold border-b border-gray-200 dark:border-gray-700">
                  <tr>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700/40">
                  {record.attendance_entries.map((a, i) => {
                    ...
                    return (
                      <tr key={i} className={`transition-colors ${absent ? "opacity-50" : "hover:bg-gray-50/50 dark:hover:bg-gray-700/20"}`}>
                        <td className="px-3 py-2 font-medium text-gray-800 dark:text-gray-200">{a.worker_name}</td>
                        <td className="px-3 py-2 text-gray-400 font-mono">{a.worker_id}</td>
                        <td className="px-3 py-2">
                          <span className="px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 font-medium">
                            {a.category || "—"}
                          </span>
                        ...
                        <td className="px-3 py-2 text-gray-400 italic">{a.remark || "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
                <tfoot className="bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
                  <tr>
                    <td colSpan="3" className="px-3 py-2 text-right text-gray-500 font-semibold uppercase text-[10px] tracking-wider">
                      Totals
                    </td>
                    <td className="px-3 py-2 text-right font-bold text-green-600">₹{fmt(amount)}</td>
...
const Chip = ({ label, color }) => (
  <span className={`px-2 py-0.5 rounded-md text-xs font-semibold ${CHIP_COLORS[color] || CHIP_COLORS.blue}`}>
    {label}
  </span>
);
...
const CARD_COLORS = {
  ...
  indigo: { wrap: "bg-indigo-50 dark:bg-indigo-900/20", icon: "text-indigo-600" },
  purple: { wrap: "bg-purple-50 dark:bg-purple-900/20", icon: "text-purple-600" },
...
const SummaryCard = ({ icon, color, label, value }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 flex items-center gap-3 shadow-sm">
      <div className={`p-2 rounded-lg shrink-0 ${c.wrap} ${c.icon}`}>{icon}</div>
      <div>
